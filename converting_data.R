...
save(list = c('images.lab', 'images.rgb', 'labels'), file = 'CIFAR_10_train.Rdata')
...
for(i in 1:num.images) {
  ...
  b <- as.integer(readBin(to.read, raw(), size=1, n=1024, endian="big"))
  index <- i
  test.rgb[[index]] = data.frame(r, g, b)
  test.lab[[index]] = l+1
}
close(to.read)

save(list = c('test.lab', 'test.rgb'), file = 'CIFAR_10_test.Rdata')
